...
try {
    ...
    switch ($action) {
        ...
        case 'login':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($response->success) {
                    ...
                    if ($role === 'admin') header('location: ' . $baseUrl . '/adminIndex.php?action=viewAllUsers');
                    else if ($role === 'nurse') header('location: ' . $baseUrl . '/nurseIndex.php?action=viewAllAppointments');
                    else if ($role === 'doctor') header('location: ' . $baseUrl . '/doctorIndex.php?action=viewAllAppointments');
                    else header('Location: ' . $baseUrl . '/index.php?action=viewAllAppointments');
                } else {
        ...
        case 'saveRegister':
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if ($response->success) {
                    ...
                    $_SESSION['error'] = $response->message;
                    $_SESSION['old'] = $data;
                    header("Location: " . $baseUrl . "/views/register.php");
                }
                exit;
            }
            break;
        ...
        case 'logout':
            session_destroy();
            header("Location: " . $baseUrl . "/views/patientDashboard.php");
            break;
