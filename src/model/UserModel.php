...
class UserModel
{
    ...
    public function Register($data) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO users (first_name, last_name, email, phone_number, gender, password, role, profile_picture)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        );
        $hashedPassword = password_hash($data["password"], PASSWORD_DEFAULT);
        $profilePicture = isset($data['profile_picture']) ? $data['profile_picture'] : null;

        $stmt->bind_param("ssssssss", $data['first_name'], $data['last_name'], $data['email'], $data['phone_number'], $data['gender'], $hashedPassword, $data['role'], $profilePicture);

        if ($stmt->execute()) {
            $user_id = $stmt->insert_id;
            if ($data['role'] === 'patient') {
                $this->createPatient($user_id, $data['address'], $data['date_of_birth'], $data['blood_type'], $data['emergency_contact']);
